        ...
        # Button to trigger calculation of Pfi, Pfo, D, Ync, Ycl and Overall yield
            if st.button("Analyse Pfi, Pfo, D, Ync, Ycl and Overall yield"):
                ...
                try:
                    ...
                    # Calculate pfi_value using the given formula
                    overall_yield_value = non_clustered_yield_value * clustered_yield_value  # Formula: =H7*I7

                    # Update or insert "Pfi" value in the DataFrame for the selected stage
                    overall_yield_value_row_idx = edited_data[edited_data['Data Points'] == "Overall yield"].index

                    if not overall_yield_value_row_idx.empty:
                        # If "Overall yield" already exists, update its value
                        edited_data.at[overall_yield_value_row_idx[0], selected_stage] = overall_yield_value
                    else:
                        # If "Overall yield" doesn't exist, create a new row for it
                        new_row_oy = pd.DataFrame({col: [np.nan] for col in edited_data.columns})
                        new_row_oy.at[0, 'Data Points'] = "Overall yield"
                        new_row_oy.at[0, selected_stage] = overall_yield_value
                    ...
                    # Optionally show a success message after calculation
                    st.success(f"Overall yield value calculated and updated: {overall_yield_value}")
